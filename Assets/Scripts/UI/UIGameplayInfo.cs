...
namespace SimpleFPS
{
	...
	public class UIGameplayInfo : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			int remainingTime = gameplay.RemainingTime.AsInt;
			...
			DoubleDamage.SetActive(gameplay.IsDoubleDamageActive);
			RemainingTime.gameObject.SetActive(gameplay.State > EGameplayState.Skirmish);

			ShowGameplayTime(remainingTime);

			var data = _gameUI.Frame.ResolveDictionary(gameplay.PlayerData);
			if (data.TryGetValue(_gameUI.Context.LocalPlayer, out PlayerData playerData))
			{
